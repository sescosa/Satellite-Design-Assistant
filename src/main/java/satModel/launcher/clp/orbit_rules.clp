...
(defrule calculate-orbit-semimajor-axis-sat
    ...
    ;;(declare (salience 20))
    ?f <- (LAUNCH-check (orbit-altitude# ?orb-alt&~nil)
...
(defrule calculate-orbit-period
    ...
    (declare (salience 20))
    ?f <- (LAUNCH-check (orbit-semimajor-axis ?a&~nil)
                                   (orbit-period# nil) )
    =>
...
(defrule estimate-sun-angle 
    ...
    ;;(declare (salience 20))
    ?sat <- (LAUNCH-check (worst-sun-angle nil) )
    =>
...
(defrule estimate-fraction-of-sunlight
    (declare (salience 20))
    ?sat <- (LAUNCH-check (orbit-semimajor-axis ?a&~nil) (fraction-sunlight nil))
    =>
